...
declare namespace $_$twiz {
    ...
    function track(//http://docs.aws.amazon.com/AmazonS3/latest/API/mpUploadUploadPart.html
    value: any, filename: string, offset: number): any;
}
import { SignedS3AWSRequest } from "./SignedS3AWSRequest";
import { Global } from "./Global";
import { PARTS_MONITOR_INTERVAL_MS, COMPLETE, ABORTED, PAUSED, CANCELED, EVAPORATING, ERROR, PAUSING } from "./Constants";
...
class PutPart extends SignedS3AWSRequest {
    ...
    getPartMd5Digest() {
        ...
        return new Promise((resolve, reject) => { $_$twiz("resolve", resolve, 1314, "/Users/matheus.moreira/Projetos/open-source/EvaporateJS/codemod/output/PutPart.ts", "{\"arrow\":true}"); $_$twiz("reject", reject, 1322, "/Users/matheus.moreira/Projetos/open-source/EvaporateJS/codemod/output/PutPart.ts", "{\"arrow\":true}"); if (self.con.computeContentMd5 && !part.md5_digest) {
            self.getPayload().then(data => {
                $_$twiz("data", data, 1424, "/Users/matheus.moreira/Projetos/open-source/EvaporateJS/codemod/output/PutPart.ts", "{\"arrow\":true,\"parens\":[1420,1424]}");
        ...
        } }).then(md5_digest => { $_$twiz("md5_digest", md5_digest, 1971, "/Users/matheus.moreira/Projetos/open-source/EvaporateJS/codemod/output/PutPart.ts", "{\"arrow\":true,\"parens\":[1961,1971]}"); if (md5_digest) {
    ...
    send() { if (this.part.status !== COMPLETE &&
        ![ABORTED, PAUSED, CANCELED].includes(this.fileUpload.status)) {
        Global.l.d("uploadPart #", this.partNumber, this.attempts === 1 ? "submitting" : "retrying");
    ...
    onProgress(evt) { $_$twiz("evt", evt, 3265, "/Users/matheus.moreira/Projetos/open-source/EvaporateJS/codemod/output/PutPart.ts", "{}"); if (evt.loaded > 0) {
    ...
    stalledPartMonitor() {
        ...
        return function () {
            ...
            if (![EVAPORATING, ERROR, PAUSING, PAUSED].includes(self.fileUpload.status) &&
                self.part.status !== ABORTED &&
                self.part.loadedBytes < this.size) {
                if (lastLoaded === self.part.loadedBytes) {
                    Global.l.w("Part stalled. Will abort and retry:", self.partNumber, decodeURIComponent(self.fileUpload.name));
                    self.abort();
    ...
    errorExceptionStatus() { return [CANCELED, ABORTED, PAUSED, PAUSING].includes(this.fileUpload.status); }
    delaySend() {
    ...
    errorHandler(reason) {
        $_$twiz("reason", reason, 4815, "/Users/matheus.moreira/Projetos/open-source/EvaporateJS/codemod/output/PutPart.ts", "{}");
        clearInterval(this.stalledInterval);
    ...
    streamToArrayBuffer(stream) { $_$twiz("stream", stream, 5467, "/Users/matheus.moreira/Projetos/open-source/EvaporateJS/codemod/output/PutPart.ts", "{}"); return new Promise((resolve, reject) => {
        $_$twiz("resolve", resolve, 5502, "/Users/matheus.moreira/Projetos/open-source/EvaporateJS/codemod/output/PutPart.ts", "{\"arrow\":true}");
        $_$twiz("reject", reject, 5510, "/Users/matheus.moreira/Projetos/open-source/EvaporateJS/codemod/output/PutPart.ts", "{\"arrow\":true}");
        // stream is empty or ended
        if (!stream.readable) {
            return resolve([]);
        }
        let arr = new Uint8Array(Math.min(this.con.partSize, this.end - this.start));
        let i = 0;
        ...
        function onData(data) {
            $_$twiz("data", data, 5894, "/Users/matheus.moreira/Projetos/open-source/EvaporateJS/codemod/output/PutPart.ts", "{}");
        ...
        function onEnd(err) {
            $_$twiz("err", err, 6053, "/Users/matheus.moreira/Projetos/open-source/EvaporateJS/codemod/output/PutPart.ts", "{}");
            if (err) {
                reject(err);
            }
            else {
    ...
    payloadFromStream() {
        const stream = this.con.readableStreamPartMethod(this.fileUpload.file, this.start, this.end - 1);
        return new Promise((resolve, reject) => {
            $_$twiz("resolve", resolve, 6931, "/Users/matheus.moreira/Projetos/open-source/EvaporateJS/codemod/output/PutPart.ts", "{\"arrow\":true}");
            $_$twiz("reject", reject, 6939, "/Users/matheus.moreira/Projetos/open-source/EvaporateJS/codemod/output/PutPart.ts", "{\"arrow\":true}");
            const streamPromise = this.streamToArrayBuffer(stream);
            streamPromise.then(data => { $_$twiz("data", data, 7038, "/Users/matheus.moreira/Projetos/open-source/EvaporateJS/codemod/output/PutPart.ts", "{\"arrow\":true,\"parens\":[7034,7038]}"); resolve(data); }, reject);
        });
    }
    payloadFromBlob() {
        ...
        if (this.con.computeContentMd5) {
            return new Promise(resolve => {
                $_$twiz("resolve", resolve, 7824, "/Users/matheus.moreira/Projetos/open-source/EvaporateJS/codemod/output/PutPart.ts", "{\"arrow\":true,\"parens\":[7817,7824]}");
    ...
    getStartedPromise() { return this.started.promise; }
}
